...
func (ev *evalCmd) Eval(ctx context.Context, queryEngine *promql.Engine, queryable storage.Queryable) error {
	q, err := queryEngine.NewInstantQuery(queryable, nil, ev.expr, ev.start)
	if err != nil {
		return err
	}
	defer q.Close()
	...
	q, err = queryEngine.NewRangeQuery(queryable, nil, ev.expr, ev.start.Add(-time.Minute), ev.start.Add(time.Minute), time.Minute)
	if err != nil {
		return err
	}
	rangeRes := q.Exec(ctx)
